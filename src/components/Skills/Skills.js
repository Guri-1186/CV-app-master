...
function Skills() {
  ...
  if (localData) {
    content = localData.map((elem) => (
      <div
        className="skills-elem"
        key={elem.skills.id}
        style={{ width: `${elem.skills.range}%` }}
      >
        <span>{elem.skills.name}</span>
      </div>
    ));
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <input
          className={
            formik.touched.range && formik.errors.range ? 'input-error' : ''
          }
          id="range"
          name="range"
          type="text"
